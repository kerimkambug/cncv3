      ...
      <div className="preset-list">
        ...
        {visiblePresets.map((p) => (
          <div key={getPresetId(p)} className={`preset-card${activeId === getPresetId(p) ? ' active' : ''}`} onClick={() => setPreviewPreset(p)}>
            ...
            <div className="preset-card-info"><strong>{p.name}</strong><span>{p.category === 'kapi' ? 'Kapı' : 'Kapak'} · {p.rows?.length || 0} işlem</span><small>{(p.rows || []).filter((row) => row.operation === 'derz').length} derz / {(p.rows || []).filter((row) => row.operation !== 'derz').length} offset</small><small className="dxf-badge">DXF kontrolü · {Number(p.previewWidth || 600).toFixed(0)} × {Number(p.previewHeight || 600).toFixed(0)} mm</small><button type="button" className="preset-select-button" onClick={(event) => { event.stopPropagation(); handleLoad(p); }}>Seç</button><button type="button" className="preset-dxf-button" onClick={(event) => { event.stopPropagation(); downloadDxf(p); }}>DXF indir</button></div>
